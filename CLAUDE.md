...
- Plan files / 计划文件：`plans/NNN-short-name.md`（带编号）
- Reflection files / 复盘文件：`reflections/NNN-short-name.md`（编号对应计划）
- Use `[[]]` wiki-links to cross-reference between files (Obsidian-compatible, not GitHub-renderable) / 使用 `[[]]` 双链语法交叉引用文件（Obsidian 兼容，GitHub 不会渲染）
