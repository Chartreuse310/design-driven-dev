...
- Long-term goals / 长期目标：`plans/goals/NNN-name.md`
- Short-term plans / 短期目标：`plans/NNN-MM-name.md`（前三位是所属长期目标编号）
- Reflection files / 复盘文件：`reflections/NNN-MM-name.md`（编号对应计划）
- All files use YAML frontmatter (date/status/tags/goal/plan as applicable) / 所有文件使用 YAML frontmatter
- Journal entries act as indexes — they link to notes/problems/projects, not duplicate content / 日记条目作为索引——链接到笔记/问题/项目，而不是复制内容
